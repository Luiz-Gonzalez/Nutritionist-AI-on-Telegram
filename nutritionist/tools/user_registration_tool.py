    ...
    def _run(
        ...
        if not name:
            raise AttributeError(
                "Os atributos inseridos na Tool UserRegistration não podem ser vazios."
            )
        try:
            user_data = {
            ...
            user = self.user_repo.get_user_by_telegram_id(telegram_id=telegram_id)
            if user:
            ...
            return f"Erro ao registra o usuário {str(err)}"
