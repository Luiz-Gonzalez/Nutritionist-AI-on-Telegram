...
class DietPlanRepository(BaseRepository):
    ...
    def create_diet_plan(self, user_id: int, plan_details: str) -> DietPlan:
        ...
        self.diet_plan_table.insert(
            json.loads(new_diet_plan.model_dump_json())
        )  # model_dump: transforma o objeto em um dicionário / json transforma o dicionario em um json
    ...
    def get_latest_diet_plan_for_user(self, user_id: int) -> Optional[DietPlan]:
        ...
        if not plans:
            return None
        latest_plan = sorted(plans, key=lambda plan: plan["created_at"], reverse=True)[
            0
        ]
    ...
    def update_diet_plan(self, plan_id: int, plan_details: str) -> Optional[DietPlan]:
        DietPlanQuery = Query()
        self.diet_plan_table.update(
            {"details": plan_details}, DietPlanQuery.id == plan_id
        )
